...
public class AHPFreqWeightedOpponentModel extends AbstractOpponentModel {
    ...
    public AHPFreqWeightedOpponentModel(Domain domain, List<Action> history, PartyId me) {
    ...
    private AHPFreqWeightedOpponentModel initializeModel(List<Action> history) {
        ...
        this.generateValueMatrices(this.subFreqModel);
    ...
    private Table<Value, Value, Double> comparePairs(Map<Value, Double> frequencies) {
        Collection<Value> allValues = frequencies.keySet();
        Table<Value, Value, Double> result = HashBasedTable.create();
        for (Value issue1 : allValues) {
            for (Value issue2 : allValues) {
                double ratio = frequencies.get(issue1) / frequencies.get(issue2);
                result.put(issue1, issue2, ratio);
            }
        }
        return result;
    ...
    private void generateValueMatrices(FrequencyOpponentModel subFreqModel2) {
        ...
        INDArray matrixForm;
        for (String issueString : this.getDomain().getIssues()) {
            List<DiscreteValue> allValues = StreamSupport
                    .stream(this.getDomain().getValues(issueString).spliterator(), false).map(v -> (DiscreteValue) v)
                    .collect(Collectors.toList());
            Map<Value, Double> allCounts = subFreqModel2.getCounts(issueString).entrySet().stream()
            ...
            Table<Value, Value, Double> cntMatrix = this.comparePairs(completeAllCounts);
            vals = cntMatrix.values().stream().mapToDouble(e -> e).toArray();
            shape = new int[] { cntMatrix.rowKeySet().size(), cntMatrix.columnKeySet().size() };
